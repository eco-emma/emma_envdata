# Steps to run the project on CCR (for users with access to the Adam Wilson Lab CCR account)

# Connect to CCR and request an interactive job

ssh vortex.ccr.buffalo.edu
salloc --cluster=faculty --qos=adamw --partition=adamw \
       --job-name=InteractiveJob --nodes=1 --ntasks=16 \
       --mem=250G -C INTEL --time=24:00:00
...
cd /projects/academic/adamw/projects/emma/emma_envdata
sbatch --cluster=faculty run_tar_make.sh