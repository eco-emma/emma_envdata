...
get_climate_chelsa <- function(
  ...
  for (idx in 1:nrow(bio_metadata)) {
    ...
    robust_download_file(
      ...
      destfile = file.path(temp_directory, paste("CHELSA_bio", sprintf("%02d", idx), 
    ...
    # Load, crop, and mask
    rast_i <- terra::rast(file.path(temp_directory, paste("CHELSA_bio", sprintf("%02d", idx), 
                                                            "_1981-2010_V.2.1.tif", sep = "")))
    rast_i <- terra::crop(x = rast_i, y = ext(domain_tf))
    rast_i <- terra::mask(rast_i, mask = terra::vect(domain_tf))
