...
process_release_stable_data <- function(temp_directory = "data/temp/processed_data/static/",
  ...
  # process data
    ...
    for(i in 1:length(raster_list$file_name)){
      ...
      file.path(temp_directory,raster_list$file_name[i]) |>
        ...
        write_parquet(sink = file.path(temp_directory,paste(raster_list$file_name[i],".parquet",sep = "")),
    ...
    # mutate(cellID = row_number()) %>%
    # filter(SA_NLC_2020_GEO.tif != 0) %>%
    # write_parquet(sink = file.path(temp_directory,"stable_data.parquet"),
    #               compression = "gzip",
  ...
  # Release
    ...
    #
    # pb_upload(file = file.path(temp_directory,"stable_data.parquet"),
    #           repo = "AdamWilsonLab/emma_envdata",
    #           tag = output_tag,
    #           show_progress = TRUE)
    #
