...
#' @description this function takes in tif file from the input, converts them to tidy format, and saves as .parquet
...
process_dynamic_data_to_parquet <- function(input_dir = "data/raw_data/ndvi_modis/",
  ...
  # figure out which files have been processed
    ...
    output_files <-
      list.files(path = output_dir, pattern = ".parquet", full.names = TRUE) %>%
  ...
  # process the files that haven't been done yet
    ...
    # Process ith file
      ...
      all_files[i] |>
        ...
        drop_na() %>%
        write_parquet(sink = paste(output_dir, integer_date_i, ".parquet", sep = ""),
                      compression = "gzip")
