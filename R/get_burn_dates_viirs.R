  ...
  task <- tryCatch(
    ...
    error = function(e) {
      ...
      if (grepl("exceeded.*maximum.*requests|too many requests",
        ...
        warning(
          "AppEEARS daily request limit reached for VIIRS burn date ",
...
download_burn_date_viirs_geotiff <- function(
  ...
  if (startsWith(task_id, "rate_limited:")) {
    ...
    warning("Rate-limited sentinel for VIIRS burn date ", yyyymm_rl,
    ...
    skip_file <- file.path(marker_dir_rl, paste0("burn_viirs_", yyyymm_rl, ".skip"))
    writeLines(
