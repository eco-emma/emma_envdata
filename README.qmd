...
# Load upstream targets to ensure readme runs after all uploads complete.
# The raster-map chunks read GitHub release URLs via /vsicurl/, so the
# upload targets must finish before this document renders.
tar_load(c(
  validate_stac, domain_geoparquet, release_manifest,
  upload_vi_modis_grid, upload_vi_viirs_grid,
  upload_burn_modis_grid, upload_burn_viirs_grid,
  upload_fireage, upload_static
))

# Set options
readme_img_dir <- "data/target_outputs/readme_img"
dir.create(readme_img_dir, recursive = TRUE, showWarnings = FALSE)

knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = paste0(readme_img_dir, "/"),
  dev = "png",
  dpi = 150
)

# GitHub release base URL for README figures
readme_assets_url <- "https://github.com/eco-emma/emma_envdata/releases/download/readme-assets"
```
...
network_html <- file.path(readme_img_dir, "network.html")
network_png  <- file.path(readme_img_dir, "network.png")

tar_visnetwork(
  targets_only = TRUE,
  label = c("time", "size", "branches")
) %>%
  htmlwidgets::saveWidget(file = network_html, selfcontained = TRUE)

# Take screenshot (requires Chromium; skip gracefully if unavailable)
if (requireNamespace("webshot2", quietly = TRUE)) {
  tryCatch(
    webshot2::webshot(network_html, network_png, vwidth = 1200, vheight = 800),
    error = function(e) message("webshot2 screenshot skipped: ", conditionMessage(e))
  )
}
```

![Pipeline Network](`r paste0(readme_assets_url, "/network.png")`)
...
```{r temporal-coverage, fig.width=10, fig.height=4, include=FALSE}
...
![](`r paste0(readme_assets_url, "/temporal-coverage-1.png")`)
...
![](`r paste0(readme_assets_url, "/evi-map-1.png")`)

```{r evi-map, fig.width=10, fig.height=6, include=FALSE}
...
if (length(vi_items) > 0) {
  ...
  # Wrapped in tryCatch: if the release asset is not yet available,
  # skip the plot gracefully rather than halting the render.
  evi_rast <- tryCatch(
    rast(paste0("/vsicurl/", evi_url)),
    error = function(e) {
      message("Could not load EVI raster (", evi_url, "): ", e$message)
      NULL
    }
  )
...
![](`r paste0(readme_assets_url, "/fire-age-map-1.png")`)

```{r fire-age-map, fig.width=10, fig.height=6, include=FALSE}
...
if (!is.null(recentburn_json)) {
  ...
  # Wrapped in tryCatch: skip gracefully if the release asset is not yet available.
  fire_rast <- tryCatch(
    rast(paste0("/vsicurl/", fire_url))[[1]],
    error = function(e) {
      message("Could not load fire-age raster (", fire_url, "): ", e$message)
      NULL
    }
  )
}

if (!is.null(recentburn_json) && !is.null(fire_rast)) {
...
![](`r paste0(readme_assets_url, "/elevation-map-1.png")`) ![](`r paste0(readme_assets_url, "/temp-map-1.png")`) ![](`r paste0(readme_assets_url, "/precip-map-1.png")`)

![](`r paste0(readme_assets_url, "/soil-map-1.png")`) ![](`r paste0(readme_assets_url, "/clouds-map-1.png")`) ![](`r paste0(readme_assets_url, "/topo-map-1.png")`)

::: {layout-ncol=3}

```{r elevation-map, fig.width=4, fig.height=4, include=FALSE}
# Read elevation COG
...
```{r temp-map, fig.width=4, fig.height=4, include=FALSE}
...
```{r precip-map, fig.width=4, fig.height=4, include=FALSE}
...
```{r soil-map, fig.width=4, fig.height=4, include=FALSE}
...
```{r clouds-map, fig.width=4, fig.height=4, include=FALSE}
# Read clouds COG
...
```{r topo-map, fig.width=4, fig.height=4, include=FALSE}
...
if (file.exists(validation_file)) {
  ...
  validation_summary <- tribble(
    ...
    "Validation Status", if(isTRUE(validation$summary$broken_links == 0)) "✅ PASS" else "❌ FAIL"
  )
